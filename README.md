...
- `-user` и `-password` — учётная запись, которую сервер заводит при старте, если её ещё нет. Пустые оба и пустая база: сервер открыт, как в локальной сети. Те же значения можно положить в `KIDS_MUSIC_USER` и `KIDS_MUSIC_PASSWORD`. Пароль хранится как bcrypt-хеш в `database.sqlite` в каталоге состояния.
...
После входа браузер хранит cookie `session`. В ней случайный номер, не имя и не пароль: сервер по этому номеру находит учётную запись и пускает только активные.
